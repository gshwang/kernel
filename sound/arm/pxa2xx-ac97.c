...
static unsigned short pxa2xx_ac97_read(struct snd_ac97 *ac97, unsigned short reg)
{
	...
	unsigned long irq_flags;								// [FA LDS] 
	...
	local_save_flags( irq_flags );							// [FA LDS]
	gsr_bits = 0;
	local_irq_restore( irq_flags );							// [FA LDS]
	val = *reg_addr;
...
static int __devinit pxa2xx_ac97_probe(struct platform_device *dev)
{
	...
	ret = request_irq(IRQ_AC97, pxa2xx_ac97_irq, SA_INTERRUPT, "AC97", NULL);
	if (ret < 0)
		goto err;
