...
static int cs4202_init(struct ac97_codec *codec);   // [FALinux] STDMM EM-AU1200
...
static struct ac97_ops cs4202_ops = { NULL, eapd_control, NULL};   
...
static const struct {
    ...
    {0x43525900, "Cirrus Logic CS4202", &cs4202_ops},   
...
static int cs4202_init( struct ac97_codec * codec )
{
   int    loop, val;
   u32    ascr;
   ...
   {
       codec->codec_write(codec, AC97_MASTER_VOL_STEREO, 0x0a0a );
       val = codec->codec_read(codec, AC97_MASTER_VOL_STEREO);

       if ( val == 0x0a0a )
       {
           ascr = codec->codec_read(codec, AC97_EXTENDED_STATUS );
           codec->codec_write(codec, AC97_EXTENDED_STATUS, ascr| 0x0001 );
           return 0;
       }
   ...
   printk( "CS4202 is not ready ... error!\n" );
   return 0;
}
...
// falinux ======================
void falinux_ac97_reg_write( unsigned char reg, unsigned short val )
{
	if (CDC)
	{
		CDC->codec_write(CDC, reg, val);
	}
	else
	{
		printk( "not exist codec\n");	
	}
}
unsigned int falinux_ac97_reg_read( unsigned char reg )
{
	if (CDC)
	{
		return CDC->codec_read(CDC, reg );
	}
	else
	{
		printk( "not exist codec\n");	
...
EXPORT_SYMBOL_GPL( falinux_ac97_reg_write );
EXPORT_SYMBOL_GPL( falinux_ac97_reg_read );
//==========================================================================
