...
/* Function prototypes */
static __inline__ int copy_to_user_with_conversion(struct audio_state_t *state, const char *to, const char *from, int toCount);
static __inline__ int copy_from_user_with_conversion(struct audio_state_t *state, const char *to, const char *from, int toCount);

static void audio_dma_start(struct audio_state_t *state, struct audio_stream_t *stream);
static void audio_dma_pause(struct audio_state_t *state, struct audio_stream_t *stream);
static void audio_prime_dma(struct audio_state_t *state, struct audio_stream_t *stream);
	...
	if (stream->buffers) {
		for (frag = 0; frag < stream->nbfrags; frag++) {
			...
			dma_free_coherent(NULL, stream->buffers[frag].master,
					  ...
					  stream->buffers[frag].dma_addr);
#else
			dma_free_writecombine(NULL, stream->buffers[frag].master,
					  stream->buffers[frag].start,
					  stream->buffers[frag].dma_addr);
#endif
		}
				  ...
				  struct audio_stream_t *stream)
{
	...
	if ((stream->requested_fragsize != 0) && (stream->requested_nbfrags != 0)) 
	{
		stream->nbfrags = (stream->requested_fragsize * stream->requested_nbfrags * stream->dma2usr_ratio) / stream->fragsize;
	}
	...
	memset(stream->buffers, 0, sizeof(struct audio_buf_t) * stream->nbfrags);
	...
	for (frag = 0; frag < stream->nbfrags; frag++) 
	{
		...
		if (!dmasize) {
			...
			do {
				...
				//                        dmasize, &dmaphys);
#if 0
				DPRINTK("dma_alloc_coherent : %08x\n", &dmaphys);
				dmabuf = dma_alloc_coherent(NULL,
							    dmasize, &dmaphys,
							    GFP_KERNEL |
							    GFP_DMA);
#else
				DPRINTK("dma_alloc_writecombine : %08x\n", &dmaphys);
			  	dmabuf = dma_alloc_writecombine(NULL, 
			  									dmasize,
												&dmaphys, 
												GFP_KERNEL|GFP_DMA);
#endif
 ...
 *  8 bit unsigned
 */
static int audio_write(struct file *file, const char *user_buffer, size_t src_count, loff_t * ppos) 
{
	const char *user_buffer_start = user_buffer;
	struct audio_state_t *state = (struct audio_state_t *)file->private_data;
	struct audio_stream_t *stream = state->output_stream;
...
void audio_dma_pause(struct audio_state_t *state, struct audio_stream_t *stream)
{
	...
	ep93xx_dma_pause(stream->dmahandles[0], stream->NumDmaChannels, stream->dmahandles);
 ...
 */
static int audio_read(struct file *file, char *user_buffer, size_t count, loff_t * ppos) 
{
...
static int audio_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	for (i = 0; i < stream->nbfrags; i++) {
		...
		ret = remap_pfn_range(vma, vma_addr, dma_buffer->dma_addr, dma_buffer->master, vma->vm_page_prot);
...
//			local_irq_save(flags);
			...
			//                &current_frac );
#if 0
			DMA_GET_POSITION(stream->dmahandles[0], &buf_id, &total, &current_frac);
#else
			ep93xx_dma_get_position(stream->dmahandles[0], &buf_id, &total, &current_frac);
#endif
...
#if 0
			DMA_GET_POSITION(os->dmahandles[0], &buf_id, &total, &current_frac);
#else
			ep93xx_dma_get_position(os->dmahandles[0], &buf_id, &total, &current_frac);
#endif
...
int ep93xx_audio_attach
	...
	struct audio_stream_t *os = state->output_stream;
	struct audio_stream_t *is = state->input_stream;
	struct file_operations *f_op = (struct file_operations *)file->f_op;
...
//			DMA_FREE(is->dmahandles[0]);
	...
	f_op->release = audio_release;
	f_op->write = audio_write;
	f_op->read = audio_read;
	f_op->mmap = audio_mmap;
	f_op->poll = audio_poll;
	f_op->ioctl = audio_ioctl;
	f_op->llseek = no_llseek;
