...
// 설명 : 커널 커맨드라인 파싱
//------------------------------------------------------------------------------
static void fixup_partition_info( struct s3c2410_nand_set *set)
{
	char *delim_ = ",";
	int  argc;
	char *argv[256];
	...
	if ( argc >= EZBOOT_PART_MIN_NR )
	{
		for ( part_index = 0 ;  part_index < argc ; part_index++)
		{
			...
			set->partitions[ part_index ].offset = offset * SZ_1M;
			set->partitions[ part_index ].size   = size   * SZ_1M;
			
			offset += size;
		}
		
		set->nr_partitions  = argc;
	}
}
...
static void fixup_partition_info(struct s3c2410_nand_set *set)
{
	...
	if ( argc == set->nr_partitions )
	{
		...
		{
			set->partitions[0].offset = 0;
			set->partitions[0].size   = size[0]*SZ_1M;
			
			set->partitions[1].offset = size[0]*SZ_1M;
			set->partitions[1].size   = size[1]*SZ_1M;
			
			set->partitions[2].offset = (size[0]+size[1])*SZ_1M;
			set->partitions[2].size   =  size[2]*SZ_1M;
		}
	}
}
