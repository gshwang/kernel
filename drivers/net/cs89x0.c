...
#elif defined(CONFIG_MACH_EZ_X5)
  static unsigned int netcard_portlist[] __initdata = {0xF1000000+0x300, 0, 0};
  static unsigned int cs8900_irq_map[] = {IRQ_GPIO(21), 0, 0};
#elif defined(CONFIG_MACH_ESP_NS)
  static unsigned int netcard_portlist[] __initdata = {0xF1000000+0x300, 0xF1100000+0x300, 0, 0};
  static unsigned int cs8900_irq_map[] = {IRQ_GPIO(21), IRQ_GPIO(22), 0, 0};
#elif defined(CONFIG_MACH_EZ_S3C2410)
  static unsigned int netcard_portlist[] __initdata = {0xF4000000+0x300, 0, 0, 0};
...
#if defined(CONFIG_MACH_EZ_X5) || defined(CONFIG_MACH_ESP_NS) || defined(CONFIG_MACH_EZ_S3C2410)
...
#if DEBUGGING
static unsigned int net_debug = 0;// DEBUGGING;
#else
...
#if defined(CONFIG_MACH_EZ_X5)
		if ( unit == 0 )
		{
			dev->irq = cs8900_irq_map[unit];
			dev->base_addr = netcard_portlist[unit];
			reset_chip(dev);	
			cs89x0_probe1( dev, dev->base_addr, 0 );
		}
		else 
		{ 	
			free_netdev(dev);	return ERR_PTR(-ENODEV);
		}
#elif defined(CONFIG_MACH_ESP_NS)				
		if ( ( unit == 0 ) || ( unit == 1 ) )
		{
...
cs89x0_probe1(struct net_device *dev, unsigned int ioaddr, int modular)
{
	...
	{
			// write mac address
			memcpy( dev->dev_addr, ezboot_mac, 6 );
			ezboot_mac[5] ++;

			for (i=0; i<3; i++) 
			{
	    		unsigned int Addr;
				Addr = dev->dev_addr[i*2] | ( (dev->dev_addr[i*2+1] << 8) & 0xff00 );
				writereg( dev, PP_IA+i*2, Addr );
			}
...
#else
	if ((readreg(dev, PP_SelfST) & (EEPROM_OK | EEPROM_PRESENT)) == (EEPROM_OK|EEPROM_PRESENT)) 
	{
...
#if defined(CONFIG_MACH_EZ_X5) || defined(CONFIG_MACH_ESP_NS) || defined(CONFIG_MACH_EZ_S3C2410) || defined(CONFIG_SH_HICOSH4)	/* no EEPROM on HiCO, don't hazzle with it here */
	if (1) {
	...
	if (lp->chip_type != CS8900 &&
		...
		if (lp->chip_type == CS8900) {
	        ...
	        i = cs8900_irq_map[0];
#elif defined(CONFIG_MACH_EZ_X5) || defined(CONFIG_MACH_ESP_NS) || defined(CONFIG_MACH_EZ_S3C2410)
...
#endif
	{
#if defined(CONFIG_MACH_EZ_X5) || defined(CONFIG_MACH_ESP_NS) || defined(CONFIG_MACH_EZ_S3C2410) || defined(CONFIG_MACH_IXDP2351) || defined(CONFIG_ARCH_IXDP2X01) || defined(CONFIG_ARCH_PNX010X)
