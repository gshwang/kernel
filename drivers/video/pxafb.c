...
// [FALINUX]
static struct pxafb_mach_info *inf;
...
static struct pxafb_mode_info ezlcd_240_320_mode = {
   .pixclock		= 81000,
   .xres			= 240,
   .yres			= 320,
   .bpp				= 16,
...
/* useful functions */
static struct pxafb_mach_info known_lcd_panels[] = 
{
	[0] = { /* 240X320 */
   		.modes		= &ezlcd_240_320_mode, 
		.num_modes	= 1,
		.lccr0		= LCCR0_Act | LCCR0_Sngl | LCCR0_Color,
		.lccr3		= LCCR3_PCP,
	},
	[1] = { /* 480X272 */
   		.modes		= &ezlcd_480_272_mode, 
		.num_modes	= 1,
		.lccr0		= LCCR0_Act | LCCR0_Sngl | LCCR0_Color,
		.lccr3		= LCCR3_PCP,
	},
	[2] = { /* 640X480 */   
		.modes		= &ezlcd_640_480_mode, 
		.num_modes	= 1,
		.lccr0		= LCCR0_Act | LCCR0_Sngl | LCCR0_Color,
		.lccr3		= LCCR3_PCP,
	},		
...
{
	struct pxafb_info *fbi = (struct pxafb_info *)info;
//	struct pxafb_mach_info *inf = fbi->dev->platform_data;		// [FALINUX]

	inf = fbi->dev->platform_data;		// [FALINUX]
...
static void pxafb_enable_controller(struct pxafb_info *fbi)
{
	...
	if ( print_lccr )
	{
		printk("  fdadr0 0x%08x\n", (unsigned int) fbi->fdadr0);
		printk("  fdadr1 0x%08x\n", (unsigned int) fbi->fdadr1);
		printk("  reg_lccr0 0x%08x\n", (unsigned int) fbi->reg_lccr0);
		printk("  reg_lccr1 0x%08x\n", (unsigned int) fbi->reg_lccr1);
		printk("  reg_lccr2 0x%08x\n", (unsigned int) fbi->reg_lccr2);
		printk("  reg_lccr3 0x%08x\n", (unsigned int) fbi->reg_lccr3);
	}
...
static struct pxafb_info * __init pxafb_init_fbinfo(struct device *dev)
{
	struct pxafb_info *fbi;
	void *addr;
//	struct pxafb_mach_info *inf = dev->platform_data;		// [FALINUX]
	struct pxafb_mode_info *mode = inf->modes;
	int i, smemlen;

	inf = dev->platform_data;		// [FALINUX]	
...
//	struct pxafb_mach_info *inf = dev->platform_data;		// [FALINUX]
	...
	while ((this_opt = strsep(&options, ",")) != NULL) {
		if (!strncmp(this_opt, "mode:", 5)) {
		...
		} else if (!strncmp(this_opt, "panel:", 6)) {						// [FALINUX]
			...
			if ((panel_index < 0) || (panel_index >= num_panels)) {
				...
				panel_index = 2;	// 640x480 default
			}
			inf = &known_lcd_panels[panel_index];
...
{
	struct pxafb_info *fbi;
//	struct pxafb_mach_info *inf;		// [FALINUX]
	int ret;

	dev_dbg(&dev->dev, "pxafb_probe\n");

	inf = dev->dev.platform_data;		// [FALINUX]
	ret = -ENOMEM;
	fbi = NULL;
	if (!inf)
		goto failed;
	...
	// [FALINUX]
	// Backlight ON
#ifdef CONFIG_ARCH_FALINUX_PXA255
	pxa_gpio_mode(16|GPIO_OUT);
	pxa_gpio_set_value(16, 1);
#endif
#ifdef CONFIG_ARCH_FALINUX_PXA270
	...
	printk("fb%d: %s frame buffer device ( %dx%dx%d )\n", 
			...
			fbi->fb.var.xres, fbi->fb.var.yres, fbi->fb.var.bits_per_pixel);
